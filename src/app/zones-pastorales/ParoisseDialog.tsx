...
const ParoisseDialog: React.FC<ParoisseDialogProps> = ({ open, zone, onClose }) => {
    ...
    return(
        <AnimatePresence>
            {open && (
                <motion.div
                    className="fixed top-30 inset-0 z-50 flex items-center justify-center bg-background/20 backdrop-blur-sm px-2"
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                >
                    <motion.div className="relative rounded-2xl shadow-xl border border-primary/50 p-4 md:p-6 w-full max-w-6xl h-[80vh] max-h-[90vh] bg-card text-card-foreground flex flex-col md:flex-row gap-4 overflow-hidden"
                    ...
                    >
                        ...
                        <div className="flex-1 items-start px-2 md:px-4 overflow-y-auto scrollbar-thin scrollbar-thumb-primary/50 scrollbar-track-transparent">
                            <h2 className="text-xl max-w-5/6 md:text-2xl font-bold mb-4 text-center text-primary-foreground py-2 rounded-lg bg-footer">
